...
export async function POST(req, { params }) {
  try {
    ...
    if (order.orderStatus !== "DELIVERED") {
      return NextResponse.json(
        { success: false, error: "Order must be delivered before requesting return" },
        { status: 400 }
      );
    }
    ...
    if (file.size > maxSize) {
      return NextResponse.json(
        { success: false, error: "File size exceeds 50 MB limit" },
        { status: 400 }
      );
    }
    ...
    const tempPath = join(tmpdir(), `return-video-${orderId}-${Date.now()}.${file.name.split('.').pop()}`);
    ...
    try {
      ...
      const uploadResult = await new Promise((resolve, reject) => {
        cloudinary.uploader.upload(
          tempPath,
          {
            resource_type: "video",
            folder: "returns",
            public_id: `return-${orderId}-${Date.now()}`,
      ...
      await unlink(tempPath).catch(() => {});
      console.error("[Return Video Upload] Cloudinary error:", uploadError);
      return NextResponse.json(
    ...
    console.error("[Return Video Upload] Error:", error);
    return NextResponse.json(
