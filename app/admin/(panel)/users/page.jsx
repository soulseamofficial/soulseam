...
// User Row Component
function UserRow({ user, orderCount }) {
  const router = useRouter();

  const handleViewOrders = () => {
    router.push(`/admin/orders?userId=${user._id}`);
  };
      ...
      <div className="md:hidden space-y-3">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-xs text-white/60 mt-1">Registered User</p>
          </div>
          {orderCount > 0 ? (
            <button
  ...
  const fetchUsers = async (page = 1, searchTerm = "", sort = "createdAt", sortOrder = "desc") => {
    try {
      ...
      if (data.success) {
        ...
        // Fetch order counts for all users in parallel
        const counts = {};
        await Promise.all(
          fetchedUsers.map(async (user) => {
            try {
              const countRes = await fetch(`/api/admin/orders/count?userId=${user._id}`, {
                credentials: "include",
              });
  ...
  return (
    <div className="text-white">
      ...
      {loading ? (
        ...
        <div className="bg-white/10 border border-white/15 rounded-2xl p-12 text-center">
          <div className="text-white/60">No registered users found</div>
        </div>
      ) : (
        <>
          ...
          <div className="space-y-4">
            {users.map((user) => (
              <UserRow key={user._id} user={user} orderCount={orderCounts[user._id] || 0} />
            ))}
          </div>
